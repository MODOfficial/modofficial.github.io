...
function GameMode:_OnGameRulesStateChange(keys)
  ...
  if newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS and GetMapName() ~= "map5v5" and GetMapName() ~= "1vs1" then
    ...
    if itemscd == true then
        for i = 0, 14 do
            ...
            if current_item ~= nil then
                if current_item:GetAbilityName() ~= "item_tpscroll" then
                    current_item:EndCooldown()
                elseif current_item:GetAbilityName() == "item_tpscroll" then
                    current_item:StartCooldown(80)
                end
            end
        end
    elseif itemscd == false then
        for i = 0, 14 do
            local current_item = hero:GetItemInSlot(i)
            if current_item ~= nil then
                if current_item:GetAbilityName() == "item_tpscroll" then
